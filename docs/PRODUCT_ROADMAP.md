# Product Roadmap

## Active Product Lines

- `OfficeX` docx-first deliverable runtime
- document governance and orchestration
- structured `docx` transformation and controlled patching
- visual and structural audit
- publication and replayable trace

## Committed Product Branches

- document ingestion workflow with repeatable import inventories
- document generation pipeline with section-managed assembly
- programmatic `docx` execution with Microsoft Word-compatible outputs
- document revision sidecar with auditable patch execution
- hybrid review canvas with Office mirror and platform truth
- open-source editor embedding for the `docx` surface
- visual audit lane for whitespace, pagination, and layout anomalies
- release and publication discipline for canonical outputs
- platform-native archive and case-study packaging

## Incubation / Research Tracks

- Document IR with provenance-aware intermediate representation
- document pull request workflow with semantic, asset, and layout diffs
- learned human feedback memory for repeated review preferences
- parser backend abstraction and fallback orchestration
- benchmark and evaluation harnesses
- schema-driven compliance profiles
- service/API/MCP exposure
- cross-renderer validation matrix

## Archived Case Studies

- `LoopMart archived case study`
  - status: archived case study
  - role: high-intensity validation history for revision, audit, and template alignment
  - location: `/Users/nihao/Documents/Playground/archive/products/loopmart`

## Backlog Discipline

- New product branches must be recorded here before becoming long-lived work.
- Finished branches should either move into active lines or archived case studies.
- Chat-only ideas are not considered durable backlog.
