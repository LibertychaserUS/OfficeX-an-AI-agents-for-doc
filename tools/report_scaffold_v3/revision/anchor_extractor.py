...
from .common import compute_file_sha256, make_run_id, render_context_lines, short_fingerprint, summarize_text, utc_now_iso
from .issue_profiles import ParagraphAnchorRule, TableCellAnchorRule, get_issue_profile
from .models import AnchorLocation, LiveAnchorRecord, LiveAnchorSnapshot, RevisionIssueLedger
...
class ParagraphScanEntry:
    ...
    paragraph: object
    normalized_text: str
    raw_style_id: Optional[str]
    is_heading: bool


def _raw_paragraph_style_id(paragraph: object) -> Optional[str]:
...
def _is_heading_style_id(style_id: Optional[str]) -> bool:
    if not style_id:
        return False
    return HEADING_STYLE_ID_PATTERN.match(style_id) is not None


def _paragraph_style_name(paragraph: object, raw_style_id: Optional[str]) -> Optional[str]:
...
def _paragraph_anchor_id(issue_id: str, anchor_role: str, location_hint: str, text: str) -> str:
...
def _build_paragraph_anchor(
    *,
    ...
    return LiveAnchorRecord(
        candidate_path=candidate_path,
        candidate_hash=candidate_hash,
        issue_id=issue_id,
        anchor_id=_paragraph_anchor_id(issue_id, anchor_role, location_hint, anchor_text),
...
def _extract_paragraph_anchor(
    ...
    *,
    ...
    if len(matches) == 1:
        ...
        style_name = _paragraph_style_name(paragraph, entry.raw_style_id)
        text = entry.normalized_text
    ...
    first_paragraph = first_entry.paragraph
    return _build_paragraph_anchor(
        issue_id=rule.issue_id,
        anchor_role=rule.anchor_role,
        candidate_path=candidate_path,
        ...
        paragraph_index=first_entry.index,
        text=first_entry.normalized_text,
        style_name=_paragraph_style_name(first_paragraph, first_entry.raw_style_id),
        leading_context=_non_empty_context(paragraph_entries, first_entry.index, direction=-1),
        trailing_context=_non_empty_context(paragraph_entries, first_entry.index, direction=1),
    ...
    *,
    ...
    for table_index, table in enumerate(document.tables):
        ...
        for row_index, row in enumerate(table.rows[1:], start=1):
            ...
            if row_index - 1 >= 1:
                previous_row = [
                    " | ".join(normalize_text(cell.text) for cell in table.rows[row_index - 1].cells)
                ]
            ...
            matches.append(
                (table_index, row_index, rule.target_column, target_cell, previous_row, next_row)
            )
    ...
    if len(matches) == 1:
        ...
        return LiveAnchorRecord(
            ...
            candidate_hash=candidate_hash,
            issue_id=rule.issue_id,
            anchor_id=_paragraph_anchor_id(rule.issue_id, rule.anchor_role, location_hint, cell_text),
            anchor_role=rule.anchor_role,
    ...
    if not matches:
        return LiveAnchorRecord(
            ...
            candidate_hash=candidate_hash,
            issue_id=rule.issue_id,
            anchor_id=_paragraph_anchor_id(rule.issue_id, rule.anchor_role, "missing", rule.anchor_role),
            anchor_role=rule.anchor_role,
            block_kind="table_cell",
            location_hint="missing",
            location=AnchorLocation(block_kind="table_cell"),
...
def build_live_anchor_snapshot_from_rules(
    candidate_path: Path,
    source_ledger_path: Path,
    issue_ids: list[str],
    anchor_rules: list[AnchorRule],
) -> tuple[LiveAnchorSnapshot, list[str]]:
    ...
    anchors: list[LiveAnchorRecord] = []
    findings: list[str] = []

    for rule in anchor_rules:
        if isinstance(rule, ParagraphAnchorRule):
    ...
    snapshot = LiveAnchorSnapshot(
        snapshot_id=make_run_id("live-anchor-snapshot"),
        generated_at_utc=utc_now_iso(),
        candidate_path=resolved_candidate,
        candidate_hash=candidate_hash,
        issue_ledger_path=source_ledger_path.expanduser().resolve(),
...
def build_live_anchor_snapshot(
    candidate_path: Path,
    ...
    for issue_id in issue_ids:
        profile = get_issue_profile(issue_id)
        anchor_rules.extend(profile.anchor_rules)

    return build_live_anchor_snapshot_from_rules(
        candidate_path,
        issue_ledger_path,
        issue_ids,
        anchor_rules,
...
def render_live_anchor_snapshot_markdown(snapshot: LiveAnchorSnapshot, findings: list[str]) -> str:
    lines = [
        ...
        f"- Issue ledger: `{snapshot.issue_ledger_path}`",
